...
func AssigneeHallserverStep2(c *gin.Context) {
	...
	if !exists {
		log.Println("\033[1;31m[ERROR] \033[0mNot found jetztMal")
		c.String(403, "无效请求")
		return
	}
	...
	if e != nil {
		log.Println("\033[1;31m[ERROR] \033[0mNot found shop_SID to used verify")
		c.String(403, "无效请求")
		return
	}
	...
	if v != hex.EncodeToString(hasher.Sum(nil)) {
		log.Printf("\033[1;31m[ERROR] \033[0minvalid sid, remote: %s, local: %s", v, hex.EncodeToString(hasher.Sum(nil)))
		c.String(403, "无效请求")
		return
	}
	...
	if !exists {
		log.Println("\033[1;31m[ERROR] \033[0mNot found parameter mobile")
		c.String(403, "无效请求")
		return
	}
	...
	if salesAdvisorID, exists = c.GetQuery("s"); exists {
		...
		if salesAdvisorID == "" {
			log.Printf("\033[1;31m[ERROR] \033[0mCan not found sales advisor config by idcard[%s] mobile[%s]", salesAdvisorID, mobile)
		...
		if salesAdvisorID == "" {
			log.Printf("\033[1;31m[ERROR] \033[0mCan not found customer config by idcard[%s] mobile[%s]", customerID, mobile)
			c.String(403, "用户ID或手机号错误")
			return
		}
	} else {
		log.Println("\033[1;31m[ERROR] \033[0mNot found customerID or salesAdvisorID")
		c.String(403, "参数错误")
		return
	}
	...
	if _, ok := config.SalesAdvisorTemplate[salesAdvisorID]; !ok {
		log.Println("\033[1;31m[ERROR] \033[0mInvalid user params", c.Params)
