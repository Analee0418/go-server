			...
			for _, row := range rows[1:] {
				...
				if strings.Contains(filename, "销售") {
					...
					if val, err := utils.HGetRedis("mobileRegion", mobile); err == nil {
						...
						if err != nil {
							log.Printf("\033[1;33m[WARNING] \033[0mcrawl mobile region failed. %v", err)
							continue
						}
						...
						if resp.StatusCode == 200 {
							...
							if err != nil {
								log.Printf("\033[1;33m[WARNING] \033[0mread mobile region failed. %v", err)
								continue
							}
							bodystr := string(body)
					...
					if err != nil {
						log.Printf("\033[1;33m[WARNING] \033[0mInvalid goodsID in \"%s\" %v", row, err)
						continue
					}
					originalPrice, err := strconv.ParseFloat(row[2], 32)
					if err != nil {
						log.Printf("\033[1;33m[WARNING] \033[0mInvalid originalPrice in \"%s\" %v", row, err)
						continue
					}
					limitPrice, err := strconv.ParseFloat(row[3], 32)
					if err != nil {
						log.Printf("\033[1;33m[WARNING] \033[0mInvalid limitPrice in \"%s\" %v", row, err)
						continue
					}
					countdownSecond, err := strconv.ParseInt(row[4], 10, 32)
					if err != nil {
						log.Printf("\033[1;33m[WARNING] \033[0mInvalid countdownSecond in \"%s\" %v", row, err)
						continue
					}
